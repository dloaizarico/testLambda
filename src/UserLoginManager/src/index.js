...
const ENABLE_LOGINS_SUCCESSFUL = "Enable logins successful";
const MISSING_OR_EMPTY_STUDENT_ARRAY =
  "Student array expected. Array missing or empty";
...
const BATCHSIZE = 15; // for general batches - should not exceed this due to Cognito throttling limits - RATE: 50/sec so assigning 15 as the 30% of the total rate.
const DEL_BATCHSIZE = 5; // separate constant for disableStudentLogins - RATE: 25/sec so assigning 5 as 20% of the total rate.
const GET_BATCHSIZE = 50; // separate constant where docClient.batchRead() is used (Max is 100) -RATE 120/sec so assigning 42% of the total rate.
...
async function createStudentLogins(docClient, props) {
  if (await validateIfOperationCanBeRun(docClient, props.schoolId)) {
    const startDate = `${dayjs(new Date()).format("YYYY-MM-DDTHH:mm:ss.SSS")}Z`;
    ...
    // TODO: get the school students that are marked as isolated and distance education.
    const schoolStudentsWithIsolatedAndDistanceEducation =
      await getIsolatedAndDistanceEducationSchoolStudents(
        docClient,
        props.schoolId
      );
    ...
    const userIdMapForIsolatedAndDistanceEducation =
      await getUserIDMapForIsolatedAndDistanceSchoolStudentEducation(
        docClient,
    ...
    await deleteStudentLogins(
      docClient,
      props,
      false,
      schoolStudentsWithIsolatedAndDistanceEducation,
      userIdMapForIsolatedAndDistanceEducation
    ); // maybe inefficent but simple
    ...
    for (const studentBatch of studentBatches) {
      ...
      let promises = studentBatch.map((student) => {
        // TODO: Validates if the student recive special education and if the student has a user already assigned.
        // If yes, it returns return {studentId: student.id, userId: existingUserID};
        const wrappedCreatedUsers = _.throttle(function (
          docClient,
          student,
          school
        ) {
          const userID = userIdMapForIsolatedAndDistanceEducation.get(
            student.id
          );
          if (userID) {
    ...
    logErrorsToWinstom(
      "Error running the process, job is busy for the same school",
      BUSY
    );
    await createUserNotification(
      docClient,
...
/**********************************************************************
 * Create student logins for an array of new students in ONE school
 * NOTE: Students can be new to Elastik or be moving from another school
 * NOTE: Propp.studentIDS[] should contain an array of studentIDs
 * NOTE: The schoolSTudents should areleady exist so we take the names from there
 * NOTE: Unlike createStudentLogins() for a whole school
 *        - we do not update the loginEnabled bit for the school
 **********************************************************************/
async function createStudentArrayLogins(docClient, props) {
  // props.schoolId and props.studenstIDs[] - an array of studentIDs
  if (await validateIfOperationCanBeRun(docClient, props.schoolId)) {
    const startDate = `${dayjs(new Date()).format("YYYY-MM-DDTHH:mm:ss.SSS")}Z`;
    await createSchoolOperationLog(
      docClient,
    ...
    // Read the schoolStudents for the school
    let schoolStudentsAll = await getSchoolStudentsBySchoolId(
      // returns "id,userId,studentID,schoolYear,studentDeparted,createdAt",
      docClient,
      props.schoolId
    );

    // Filter out the students we want
    let schoolStudents = schoolStudentsAll.filter((schoolStudent) => {
      return props.studentIDs.includes(schoolStudent.studentID);
    });
    ...
    // Clean out any existing Cognito users and User records
    // This wont run unless we ignore the canBeRun check because createStudentArrayLogins has set the lock!
    console.time("Cleanout");
    let validateIfRunOperation = false;
    await deleteStudentArrayLogins(docClient, props, validateIfRunOperation);
    console.timeEnd("Cleanout");
    ...
    // Batch up the unique students into BATCHSIZE (15) to create the Cognito and User records
    let studentBatches = makeBatches(students, BATCHSIZE); // an array of student arrays
    logToWinstom("student batches", studentBatches.length);
    logToWinstom("student batches", studentBatches[0]);
 ...
 * *********************************************************************/
async function deleteStudentLogins(
  docClient,
  props,
  validateIfRunOperation,
  schoolStudentsWithIsolatedAndDistanceEducation,
  userIdMapForIsolatedAndDistanceEducation
) {
  if (
    await validateIfOperationCanBeRun(
      docClient,
      props.schoolId,
      validateIfRunOperation
    )
    ...
    // TODO: get two maps: 1. a map with all the userIDs of schoolStudents that are receiving isolated and distance education.
    // 2. A map or array with schoolStudents that are receiving isolated and distance education and therefore, the user should not be deleted.
    if (!schoolStudentsWithIsolatedAndDistanceEducation) {
      schoolStudentsWithIsolatedAndDistanceEducation =
        await getIsolatedAndDistanceEducationSchoolStudents(
          docClient,
          props.schoolId
    ...
    if (!userIdMapForIsolatedAndDistanceEducation) {
      userIdMapForIsolatedAndDistanceEducation =
        await getUserIDMapForIsolatedAndDistanceSchoolStudentEducation(
          docClient,
          schoolStudentsWithIsolatedAndDistanceEducation
    ...
    for (const schoolStudent of schoolStudents) {
      // TODO: AND the userID does not belong to an isolated and distance education student.
      if (
        schoolStudent.userId &&
        !userIdMapForIsolatedAndDistanceEducation.get(schoolStudent.studentID)
      ) {
    ...
    for (const userIdBatch of userIdBatches) {
      logToWinstom(
        `**********Cleaning User batch ${n++} of ${
          userIdBatches.length
        } batches`
      );
    ...
    let schoolStudentBatches = makeBatches(
      schoolStudentsWithUserIds,
      BATCHSIZE
    );
    ...
    // TODO: AND userID is not part of an isolated and distance education.

    const schoolStudentUsersWithDistanceEducation = [
      ...userIdMapForIsolatedAndDistanceEducation.values(),
    ];
    ...
    let studentUsers = users.filter(
      (user) =>
        ...
        !schoolStudentUsersWithDistanceEducation.includes(user.userId)
    );
...
/**********************************************************************
 * Delete student logins for an array of new students in ONE school
 * NOTE: Students can be new to Elastik or be moving from another school
 * NOTE: Propp.studentIDs[] should contain an array of studentIDs
 * NOTE: The schoolStudents should arleady exist
 * NOTE: The logins may or not exist at time of delete
 * NOTE: Unlike deleteStudentLogins() for a whole school
 *        - we do not update the loginEnabled bit for the school
 *        - we do not clean up orphan users
 * *********************************************************************/
async function deleteStudentArrayLogins(
  docClient,
  props,
  validateIfRunOperation // will be set to false when deleteStudentArrayLogins is called by createStudentArrayLogins
) {
  if (
    await validateIfOperationCanBeRun(
      docClient,
    ...
    // filter out the studnets we want
    let schoolStudents = schoolStudentsAll.filter((schoolStudent) => {
      return props.studentIDs.includes(schoolStudent.studentID);
    });
    logToWinstom("No of schoolStudents", schoolStudents.length);
...
const handler = async (event) => {
  ...
  switch (props.actionCode) {
    ...
    case "createStudentLogins": {
      ...
      retVal = result.err ? result.err : ENABLE_LOGINS_SUCCESSFUL;
      break;
    }
    ...
    /*********************************
     * Methods on arrays of students
     *********************************/
    case "createStudentArrayLogins": {
      if (!props.schoolId) {
        retVal = BAD_OR_NO_ATTRIBUTES;
        break;
      }
      if (!(props.studentIDs || props.studentIDs.length > 0)) {
        retVal = MISSING_OR_EMPTY_STUDENT_ARRAY;
        break;
      }
      let result = await createStudentArrayLogins(docClient, props); // new at 30/11/2023
      retVal = result.err
        ? result.err
        : `Success creating ${props.studentIDs.length} logins`;
      break;
    }
    case "deleteStudentArrayLogins": {
      if (!props.schoolId) {
        retVal = BAD_OR_NO_ATTRIBUTES;
        break;
      }
      ...
      let result = await deleteStudentArrayLogins(docClient, props); // tested 7/8/2023
      retVal = result.err
        ? result.err
        : `Success removing ${props.studentIDs.length} logins`;
      break;
    }
    /******************************
