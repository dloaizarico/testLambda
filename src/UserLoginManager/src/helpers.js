...
// Retrieve an array of school students by studentId in the current year.
async function getSchoolStudentsByStudentIdForTheCurrentYear(
  docClient,
  studentId
) {
  let schoolStudents = [];
  let params = {
    ...
    FilterExpression: "schoolYear = :schoolYear",
    ExpressionAttributeValues: {
      ":studentID": studentId,
...
async function getUserIDMapForIsolatedAndDistanceSchoolStudentEducation(
  ...
  const userIDMapForIsolatedAndDistanceEducation = new Map();
  for (let index = 0; index < schoolStudents.length; index++) {
    const schoolStudent = schoolStudents[index];

    const schoolStudentsInCurrentYear =
      await getSchoolStudentsByStudentIdForTheCurrentYear(
        docClient,
  ...
  return userIDMapForIsolatedAndDistanceEducation;
} /// end getUserIDMapForIsolatedAndDistanceSchoolStudentEducation()
...
// Read all the school students mark in the current year with the flag isolatedAndDistanceEducation
async function getIsolatedAndDistanceEducationSchoolStudents(
  docClient,
  schoolId
) {
    ...
    do {
      result = await docClient
        .query({
          ...
          FilterExpression:
            "schoolYear = :schoolYear and doesReceiveIsolatedAndDistanceEducation = :doesReceiveIsolatedAndDistanceEducation",
          ExpressionAttributeValues: {
            ":schoolID": schoolId,
            ":schoolYear": getSchoolYear(),
            ":doesReceiveIsolatedAndDistanceEducation": 1,
          },
          ProjectionExpression:
...
// it creates a transaction log for the school for the operation invoked.
...
// Defines if the lambda is currently attending an operation for the same school or not.
const validateIfOperationCanBeRun = async (
  docClient,
  schoolID,
  validateIfRunOperation
) => {
  // if it does not need to run the validation, it just returns true.
  if (validateIfRunOperation === false) {
    return true;
  }
  let schoolLogs = await getSchoolOperationsLog(docClient, schoolID);
  schoolLogs = _.orderBy(schoolLogs, "createdAt", "desc");
  if (
    (schoolLogs && schoolLogs[0] && schoolLogs[0].endDate) ||
    schoolLogs.length === 0
  ) {
    return true;
  }
...
// Query the school transaction table by schoolID index.
...
// Create the end log record when the operation is finished.
...
const createUserNotification = async (
  ...
  try {
    ...
    const input = {
      ...
      message: message
        ? message
        : `The process: ${actionCode} you started for ${schoolName} has finished.`,
      recipient: userEmail,
