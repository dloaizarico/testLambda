...
const matchStudentNamesByIndexes = (student, maps) => {
  ...
  if (foundStudents.length === 0) {
    ...
    for (let index = 0; index < foundStudents.length; index++) {
      ...
      logger.debug(`birthDateSimiliraryRatio: ${birthDateSimiliraryRatio}`);
      if (
    ...
    logger.debug(`Result: ${filteredPossibleMatches}`);
    ...
    const birthDateSimiliraryRatio = diceCoefficient(
      student.birthDate,
      foundStudents[0].birthDate
    );

    logger.debug(`birthDateSimiliraryRatio: ${birthDateSimiliraryRatio}`);
    if (birthDateSimiliraryRatio > constants.acceptedBirthdateSimilarityRatio) {
      return {
