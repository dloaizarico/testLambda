...
const handler = async (event) => {
  ...
  if(event.message){
    if(event.message.includes("EVENT")){
      eventMessage = typeof event.message === "string" ? JSON.parse(event.message.substring(7)) : event.message;
    }else{
      eventMessage = typeof event.message === "string" ? JSON.parse(event.message) : event.message;
    }
  }else{
  ...
  switch (input.method) {
    case "matchStudentByNameUsingIndexes": {
      response = matchStudentByNameUsingIndexes(input.student, input.matchingList);
      break;
    }
    case "matchStudentByNameWithSomeEquality": {
      response = matchStudentByNameWithSomeEquality(input.student, input.matchingList);
      break;
    }
    case "matchStudentByNameWithStrictEquality": {
      response = matchStudentByNameWithStrictEquality(input.student, input.matchingList);
      break;
    }
    default: {
      unknowMethod = true;
