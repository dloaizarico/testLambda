...
const processTextactResult = async (textractClient, jobId) => {
  logger.debug("about to get textract");
  let nextToken = null;
  ...
  try {
    ...
    do {
      const result = await textractClient
        ...
        .promise();
      logger.debug(`got textract ${result.NextToken}`);
      if (result?.Blocks) {
      ...
      await sleep(5000);
      logger.debug("waiting to avoid reaching the textract limit");
    } while (nextToken);
    ...
    try {
      ...
      for (let index = 0; index < studentsHandWritingLog.length; index++) {
        ...
        if (
          ...
          !studentHandwritingLog.studentID.includes("unidentified")
        ) {
